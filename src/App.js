...
const App = () => {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:3001/game").then((gameResponse) => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post("http://localhost:3001/players", newPlayer).then((response) => {
      setPlayers(players.concat(response.data))
      setNameField("")
    })
      ...
      .then((response) => {
        ...
        promise.then((response) => {
          ...
          if (gameObject.game === 1) {
            axios
              .post("http://localhost:3001/game", gameObject)
              .then((response) => {
                setGame(response.data.game)
              })
          } else {
            axios
              .put("http://localhost:3001/game/1", gameObject)
              .then((response) => {
                setGame(response.data.game)
  ...
  const handleDelete = (e) => {
    e.preventDefault()
    axios
      .delete(`http://localhost:3001/players/${e.target.id}`)
      .then((response) => {
        const promise = axios.get("http://localhost:3001/players")
        promise.then((response) => {
          setPlayers(response.data)
    ...
    <div className="App">
      ...
      >
        <form onSubmit={handleSubmit}>
          ...
          <div style={{}}>
            <div
              style={{ flexDirection: "row", display: "flex", width: "40vw" }}
            >
              <Button
                style={{
                  height: "4em",
                ...
                variant="primary"
              >
                <strong>Uusi pelaaja bingoon</strong>
              </Button>
              <p style={{ marginLeft: "2em", fontSize: "40px" }}>
                Kierroksia: {game}
              </p>
            </div>
            <Button
            ...
            >
              <strong>Arvotaan voittaja</strong>
            </Button>
          </div>
        </form>
      </div>
      <div>
        {players !== null ? (
          ...
          <div>
            ...
            <img
              src={sadFace}
              alt="surullinen naama on nyt [Object object]"
              height="400px"
            ></img>
          </div>
        )}
